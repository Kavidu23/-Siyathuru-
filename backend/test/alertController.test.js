...
describe("Alert Controller Unit Tests", () => {
    ...
    it("should update an alert and return 200 status", async () => {
        ...
        req.body = updateData;
        const updatedAlert = { _id: alertId, ...updateData, message: "Existing message", severity: "medium" };

        Alert.findByIdAndUpdate.mockResolvedValueOnce(updatedAlert);

        await updateAlert(req, res);

        expect(Alert.findByIdAndUpdate).toHaveBeenCalledWith(alertId, updateData, { new: true, runValidators: true });
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith({
            success: true,
