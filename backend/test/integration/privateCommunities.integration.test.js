...
describe('Integration: Private Communities', () => {
  ...
  it('should enforce auth+member role on join request endpoint (401/403/200)', async () => {
    ...
    const noTokenRes = await request(app).post(`/api/private-communities/${community._id}/join`);
    expect(noTokenRes.status).toBe(401);
    expect(noTokenRes.body).toEqual({ message: 'No token provided' });

    const forbiddenRes = await leaderAgent.post(`/api/private-communities/${community._id}/join`);
    expect(forbiddenRes.status).toBe(403);
    expect(forbiddenRes.body).toEqual({ message: 'Access denied' });

    const requestRes = await memberAgent.post(`/api/private-communities/${community._id}/join`);
    ...
    expect(communityInDb.joinRequests.map((r) => String(r.user))).toContain(String(memberUser._id));
  });
  ...
  it('should allow leader to view and approve join request, updating community and user', async () => {
    ...
    const requestsRes = await leaderAgent.get(`/api/private-communities/${community._id}/requests`);
    ...
    expect(String(requestsRes.body.joinRequests[0].user._id)).toBe(String(memberUser._id));
    ...
    expect(updatedUser.joinedCommunities.map(String)).toContain(String(community._id));
  });
});
